...
siscomp<-reporte1[reporte1$PROGRAMA.EDUCATIVO == "INGENIERIA EN SISTEMAS COMPUTACIONALES",]


reporteAlumnosunir<- merge(siscomp,mostrarSolo, by = c(  "ASIGNATURA" ,"EJE","PROGRAMA.EDUCATIVO", "GRUPO" ))

siscomp <- select(siscomp,2:16)
...
for (i in  1:length(s)){
  listOfDataframe[[i]]<-siscomp[siscomp$EJE == s[i], ]
 View(listOfDataframe[[i]])
  sumaCalifs[[i]] <- c(s[i],sum(listOfDataframe[[i]]$"SUMA/CALIFICACIONES"))
  totalEst[[i]] <- c(s[i],sum(listOfDataframe[[i]]$"TOTAL DE ESTUDIANTES"))
}
...
repGest<-reporte1[reporte1$PROGRAMA.EDUCATIVO == 'INGENIERIA EN GESTION EMPRESARIAL',]

reporteAlumnosunir<- merge(repGest,mostrarSolo, by = c(  "ASIGNATURA" ,"EJE","PROGRAMA.EDUCATIVO", "GRUPO" ))


repGest <- select(repGest,2:16)


View(repGest)


s <- unique(repGest$EJE)


listOfDataframeg <- lst()
sumaCalifs <- lst()
totalEst<- lst()

for (i in  1:length(s)){
  listOfDataframeg[[i]]<-gestionem[gestionem$EJE == s[i], ]
  sumaCalifs[[i]] <- c(s[i],sum(listOfDataframeg[[i]]$"SUMA/CALIFICACIONES"))
  totalEst[[i]] <- c(s[i],sum(listOfDataframeg[[i]]$"TOTAL DE ESTUDIANTES"))
}
...
totalesxAreaGestion<- merge(areaTotalRepAp,unir,by = c("EJE"))
colnames(totalesxAreaGestion)

totalesxAreaGestion <- select(totalesxAreaGestion,"EJE","tc","te","p","estudiantesEval","totalAprobados","porcentjeAprobados","rep","porcentjeReprobados")


colnames(totalesxAreaGestion)[2] <- 'Suma total/calificaciones'
colnames(totalesxAreaGestion)[3] <- 'Suma total/Evaluados'
colnames(totalesxAreaGestion)[4] <- 'Suma total/promedio'
colnames(totalesxAreaGestion)[5] <- 'Total Estudiantes-Eval'
colnames(totalesxAreaGestion)[6] <- 'Aprobados'
colnames(totalesxAreaGestion)[7] <- '% de Aprobacion'
colnames(totalesxAreaGestion)[8] <- 'Reprobados'
colnames(totalesxAreaGestion)[9] <- '% de Reprobacion'

View(totalesxAreaGestion)
...
#*********-----------------------------**reporte de INGENIERIA EN INDUSTRIAS ALIMENTARIAS
#*
repIAl<-reporte1[reporte1$PROGRAMA.EDUCATIVO == 'INGENIERIA EN INDUSTRIAS ALIMENTARIAS',]

###----------------------
reporteAlumnosunir<- merge(repIAl,mostrarSolo, by = c(  "ASIGNATURA" ,"EJE","PROGRAMA.EDUCATIVO", "GRUPO" ))

###----------------------
repIAl <- select(repIAl,2:16)
...
#----------------
listOfDataframeal<- lst()
sumaCalifs <- lst()
totalEst<- lst()

for (i in  1:length(s)){
  listOfDataframeal[[i]]<-repIAl[repIAl$EJE == s[i], ]
  View(listOfDataframeal[[i]])
  sumaCalifs[[i]] <- c(s[i],sum(listOfDataframeal[[i]]$"SUMA/CALIFICACIONES"))
  totalEst[[i]] <- c(s[i],sum(listOfDataframeal[[i]]$"TOTAL DE ESTUDIANTES"))
}
...
totalesxAreaAlimentarias<- merge(areaTotalRepAp,unir,by = c("EJE"))
colnames(totalesxAreaAlimentarias)

totalesxAreaAlimentarias <- select(totalesxAreaAlimentarias,"EJE","tc","te","p","estudiantesEval","totalAprobados","porcentjeAprobados","rep","porcentjeReprobados")


colnames(totalesxAreaAlimentarias)[2] <- 'Suma total/calificaciones'
colnames(totalesxAreaAlimentarias)[3] <- 'Suma total/Evaluados'
colnames(totalesxAreaAlimentarias)[4] <- 'Suma total/promedio'
colnames(totalesxAreaAlimentarias)[5] <- 'Total Estudiantes-Eval'
colnames(totalesxAreaAlimentarias)[6] <- 'Aprobados'
colnames(totalesxAreaAlimentarias)[7] <- '% de Aprobacion'
colnames(totalesxAreaAlimentarias)[8] <- 'Reprobados'
colnames(totalesxAreaAlimentarias)[9] <- '% de Reprobacion'




View(totalesxAreaAlimentarias)


#****************************************************************reporte de INGENIERIA ELECTROMECANICA


View(totalesxAreaElectro)

#**************************************++++++++++++++++++++++++++++++*reporte de INGENIERIA INDUSTRIAL
repIInd<-reporte1[reporte1$PROGRAMA.EDUCATIVO == 'INGENIERIA INDUSTRIAL',]  


View(totalesxAreaIndustrial)


#********************************************************************reporte de INGENIERIA EN LOGISTICA
repILog<-reporte1[reporte1$PROGRAMA.EDUCATIVO == 'INGENIERIA EN LOGISTICA',]




repTIC<-reporte1[reporte1$PROGRAMA.EDUCATIVO == 'INGENIERIA EN TIC´S',]
